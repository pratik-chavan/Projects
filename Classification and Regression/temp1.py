...
for i in range(0, ytest.shape[0]):
    for j in range(1, k+1):
        ...
        #m1[j] = np.reshape(2,1);
        '''
        temp1 = np.dot( np.dot(tmp,cinv) ,np.mat(Xtest[i]).T ); 
        temp2 = (0.5)* np.dot(np.dot(tmp,cinv),tmp.T); 
        #temp1 = tmp*cinv*np.mat(Xtest[i]).T;
        #temp2 = (0.5)*tmp*cinv*tmp.T;
        ...
        temp = temp1 - temp2;
        '''
        temp1 = (1/ ((2*3.14168)**(Xtest.shape[1]/2.0)) * math.sqrt((np.linalg.det(cov))))
        temp2 = math.exp((-0.5)*(Xtest[i]-tmp) * cov * (Xtest[i]-tmp).T);
        temp = temp1*temp2
