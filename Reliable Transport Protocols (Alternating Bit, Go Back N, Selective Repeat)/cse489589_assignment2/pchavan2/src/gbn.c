...
int i;  
int count;  // Used to adding packets to buffer. buffer is used in possible cases of retransmission of multiple packets.
  ...
  struct pkt packet;
{
	...
	printf("A_INPUT->>>>> Packet.ACK Number = %d  Expected ACK = %d \n\n", packet.acknum, (nextseqnum-1) ); 
	if(cs == packet.checksum )
		...
		for ( i=base; i< base + WINSIZE; i++)
			...
			else if(nextseqnum <base + WINSIZE)
			{
				...
				printf("A_INPUT->>>>> i = %d \n", i);
				if(buf[i].sent==0 && buf[i].packet.seqnum!=0)
				{
				printf("A_INPUT->>>>In if while sending packet %d\n", buf[i].packet.seqnum);
	...
	if ((checksum  == packet.checksum) && (packet.seqnum == expectedseqnum))
		{
			...
			packet_most_recently_acked=packet;
...
{
	expectedseqnum=0;
	packet_most_recently_acked.acknum=0;
	packet_most_recently_acked.seqnum=0;
	for(i=0; i<20;i++)
	{
			packet_most_recently_acked.payload[i]=0;
			packet_most_recently_acked.checksum = packet_most_recently_acked.checksum + packet_most_recently_acked.payload[i];
	}		
