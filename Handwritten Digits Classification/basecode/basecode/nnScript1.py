...
def nnObjFunction(params, *args):
    ...
    training_data1 = np.hstack((training_data,ar))
    z = np.dot(training_data1,w1.T)
    z = sigmoid(z)
    z1 = np.hstack((z, ar))
    o = np.dot(z1, w2.T)
    o = sigmoid(o)
    ...
    #print J
    dlzj = np.dot(np.transpose(delta),z1) #sum of equation 9 which is used in equation 16
    dw2=np.dot(delta,w2)
    dw2_zj=z1*(1-z1)
    dlzp = np.dot(np.transpose(dw2_zj*dw2),training_data1)   #equation 12
    lamda = lambdaval
    w = sum(sum(w1*w1))+ sum(sum(w2*w2))
    Jcap = J + (lamda/(2*training_data.shape[0]))*w   #equation 15
    obj_val = Jcap
    #Make sure you reshape the gradient matrices to a 1D array. for instance if your gradient matrices are grad_w1 and grad_w2
    #you would use code similar to the one below to create a flat array
    #obj_grad = np.concatenate((grad_w1.flatten(), grad_w2.flatten()),0)
    obj_grad = np.array([])
    Jcap1 = (dlzj+lamda*w2)/training_data.shape[0]
    dlzp = np.delete(dlzp,(-1),axis=0)
    Jcap2 = (dlzp+lamda*w1)/training_data.shape[0]
