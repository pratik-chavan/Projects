...
def nnObjFunction(params, *args):
    ...
    input_bias = np.ones((784,1))
    #input_bias = input_bias.T
    print input_bias.shape
    #np.append(train_data, input_bias, axis=0)
    # print "IB" , input_bias.shape
    ib = input_bias.tolist()
    np.concatenate((training_data,input_bias))
    print "Train with bias " ,training_data.shape
    ...
    for j in range(50):
        temp = 0
        for i in range(784):
            temp += w1[[j],[i]]*training_data[[0],[i]]
        temp += w1[[j],[784]]
        sig1[len(sig1):] = sigmoid(temp)
    ...
    for j in range(10):
        ...
        for i in range(50):
            temp += w2[[j],[i]]*sig1[i]
        temp += w2[[j],[50]]
        sig2[len(sig2):] = sigmoid(temp)
    
    
    output = np.array(sig2)
    delta = output - training_label[0]
    print delta
    '''
